...
export const load: PageServerLoad = async (event) => {
  ...
  try {
    ...
    try {
      ...
      const confirmedCount = confirmationsList.filter((c: any) => c.confirmation_type === 'confirmed').length;
      const disputedCount = confirmationsList.filter((c: any) => c.confirmation_type === 'disputed').length;
      ...
      const canExtend = (isOwner || isModerator) && 
                  ['auto_expire', 'user_resolvable'].includes(hazard.expiration_type || 'user_resolvable') &&
                  !hazard.resolved_at;

      // Can submit resolution report if no report exists yet and hazard not resolved
      const canResolve = !resolutionReport && !hazard.resolved_at;
