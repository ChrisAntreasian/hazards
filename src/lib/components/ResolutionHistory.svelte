<script lang="ts">
	...
	let { report, confirmedCount, disputedCount, confirmationThreshold = 3 }: Props = $props();

	const totalVotes = $derived(confirmedCount + disputedCount);
	const confirmedPercentage = $derived(totalVotes > 0 ? Math.round((confirmedCount / totalVotes) * 100) : 0);
	const disputedPercentage = $derived(totalVotes > 0 ? Math.round((disputedCount / totalVotes) * 100) : 0);
	...
	const remaining = $derived(Math.max(0, confirmationThreshold - confirmedCount));
...
<div class="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
	...
	<div class="flex items-start justify-between gap-4">
		<div class="flex-1">
			<div class="flex items-center gap-2 mb-1">
				...
				{#if willAutoResolve}
					<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 border border-purple-300">
						<span>⌛</span>
	...
	<div class="bg-gray-50 rounded-lg p-3">
		<p class="text-sm text-gray-800 whitespace-pre-wrap">{report.resolution_note}</p>
	</div>
	...
	<div class="space-y-2">
		<div class="flex items-center justify-between text-sm">
			...
			<span class="text-gray-500">{totalVotes} vote{totalVotes !== 1 ? 's' : ''}</span>
		</div>
		...
		{#if willAutoResolve}
			<div class="bg-purple-50 border border-purple-200 rounded-lg p-3">
				<p class="text-sm text-purple-800">
					<strong>✓ Threshold met!</strong> This hazard will be automatically marked as resolved.
				</p>
			</div>
