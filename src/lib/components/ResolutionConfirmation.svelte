<script lang="ts">
	...
	let { hazardId, userConfirmation = null, onConfirmationChange }: Props = $props();
	...
	async function handleConfirm(type: 'confirmed' | 'disputed') {
		...
		try {
			const response = await fetch(`/api/hazards/${hazardId}/resolution-confirmation`, {
				method: 'POST',
				headers: {
...
<div class="space-y-3">
	...
	{:else if currentConfirmation === 'disputed'}
		<p class="text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg p-2">
