<script lang="ts">
      ...
      try {
        ...
        try {
          ...
          logger.componentError("Map", error as Error, {
            action: "load_marker_cluster_plugin",
          });
        }
        ...
        if (L!.markerClusterGroup) {
          try {
            ...
            logger.componentError("Map", error as Error, {
              action: "initialize_marker_clustering",
            });
            markerClusterGroup = null;
          }
        } else {
          logger.warn(
            "MarkerCluster plugin not available, using regular markers",
            { component: "Map" }
          );
          markerClusterGroup = null;
        }
  ...
  function updateHazardMarkers() {
    ...
    hazards.forEach((hazard: HazardMapData, index: number) => {
      try {
        if (hazard?.latitude && hazard?.longitude) {
          const categoryColor =
            categoryColors[hazard?.category_name || "Other"] ||
            categoryColors["Other"];

          const marker = L!.marker(
            [
              parseFloat(String(hazard.latitude)),
              parseFloat(String(hazard.longitude)),
            ],
            {
              icon: L!.divIcon({
