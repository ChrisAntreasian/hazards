<script lang="ts">
  ...
  $: mainContentFiles = content.files.filter(
    (f) => f.type !== "regional_notes"
  );
...
<div class="educational-content-card">
      ...
      <div class="gallery-grid">
        {#each content.images as imageUrl (imageUrl)}
          <div class="gallery-item">
            <img
              src={imageUrl}
              alt="{content.displayName} example"
              loading="lazy"
            />
          </div>
        {/each}
  ...
  <div class="main-content">
    <ContentNavigationTabs
      files={mainContentFiles}
      activeFileType={activeTab}
  ...
  {#if content.regionalContent.length > 0}
    <div class="regional-content">
      ...
      <p class="regional-description">
        This hazard may vary by region. Select your area to see
