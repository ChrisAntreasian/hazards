<script lang="ts">
	import { getTrustScoreTier, TRUST_SCORE_TIERS, type TrustScoreEvent } from '$lib/utils/trust-score';

	let { userId, events = [], limit = 50, filters = [] }: {
		userId: string;
...
<div class="trust-score-history">
	...
	{#if filteredEvents.length === 0}
		<div class="no-events">
			<p>No events found. {filters.length > 0 ? 'Try adjusting your filters.' : 'Start contributing to earn trust score!'}</p>
		</div>
	{:else}
		<div class="timeline">
			{#each Object.entries(groupedEvents) as [date, dayEvents]}
				<div class="date-group">
					<div class="date-header">
						<span class="date-label">{new Date(date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' })}</span>
					...
					<div class="events-list">
						{#each dayEvents as event}
							<div class="event-item">
								<div class="event-icon" style="color: {getEventColor(event.eventType, event.pointsChange)}">
									{getEventIcon(event.eventType)}
								</div>
								<div class="event-content">
									<div class="event-header">
										<span class="event-type">{formatEventType(event.eventType)}</span>
										<span class="event-points" style="color: {getEventColor(event.eventType, event.pointsChange)}">
