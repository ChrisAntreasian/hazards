<script lang="ts">
	...
	let { userId, score, breakdown = [], showHistory = false }: {
		userId: string;
		score: number;
	...
	let nextTierInfo = $derived.by(() => {
		const tiers = Object.entries(TRUST_SCORE_TIERS).sort(([, a], [, b]) => a.minScore - b.minScore);
	...
	let totalPositive = $derived(positiveActions.reduce((sum, b) => sum + b.totalPoints, 0));
	let totalNegative = $derived(negativeActions.reduce((sum, b) => sum + b.totalPoints, 0));
</script>
	...
	{#if nextTierInfo}
		<div class="progress-container">
			<div class="progress-bar">
				<div class="progress-fill" style="width: {progress}%; background: {tierConfig.color}"></div>
			</div>
						...
						{#each positiveActions as action}
							<div class="breakdown-item">
								<div class="item-header">
									<span class="item-type">{action.eventType.replace(/_/g, ' ')}</span>
